...
export default function InventoryPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-5">
        <h1 className="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-100">{t('inventory.title')}</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">{t('inventory.subtitle')}</p>
      </div>

      {/* summary */}
      <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-3">
        <div className="flex items-center gap-3 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400 dark:text-zinc-500">{t('inventory.products_tracked')}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400 dark:text-zinc-500">{t('inventory.low_stock')}</p>
          </div>
        </div>
        <div className="col-span-2 flex items-center gap-3 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800 md:col-span-1">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400 dark:text-zinc-500">{t('inventory.restocks_logged')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-5">
        {/* stock levels */}
        <div className="rounded-2xl bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800 xl:col-span-3">
          <h2 className="border-b border-zinc-100 dark:border-zinc-800 px-5 py-4 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('inventory.stock_levels')}</h2>
              ...
              <li key={p.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-sm font-semibold text-zinc-900 dark:text-zinc-100">{p.name}</p>
                ...
                <span className="w-12 text-end font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{p.stock}</span>
                <button onClick={() => openModal(p)} className="rounded-lg bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-brand-50 hover:text-brand-700">
                  {t('inventory.adjust')}
                </button>
              </li>
            ))}
        ...
        {/* movement history */}
        <div className="rounded-2xl bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800 xl:col-span-2">
          <h2 className="border-b border-zinc-100 dark:border-zinc-800 px-5 py-4 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('inventory.movement_history')}</h2>
          <ul className="thin-scroll max-h-[560px] divide-y divide-zinc-50 overflow-y-auto">
            ...
            {logs.map((l) => (
              <li key={l.id} className="flex items-center gap-3 px-5 py-2.5">
                ...
                <span className="w-14 text-end text-[10px] text-zinc-300 dark:text-zinc-600">{timeAgo(l.created_at)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <Modal open={!!target} onClose={() => setTarget(null)} title={`${t('inventory.adjust_stock')} ${target?.name ?? ''}`}>
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {REASONS.map((r) => (
              <button
                ...
                className={`rounded-xl border-2 py-2 text-xs font-bold transition ${reason === r.value ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-zinc-100 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400'}`}
              >
                {t(r.labelKey)}
              </button>
            ))}
          ...
          <div>
            ...
            <input value={qty} onChange={(e) => setQty(e.target.value)} type="number" min="1" className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400" />
          </div>
          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('inventory.notes')}</label>
            <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t('inventory.notes.placeholder')} className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400" />
          </div>
