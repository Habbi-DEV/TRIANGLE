  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-100">{t('menu.title')}</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">{t('menu.stats', { products: products.length, categories: categories.length })}</p>
        </div>
      ...
      {/* categories */}
      <div className="mb-6 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
        <h2 className="mb-1 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('menu.categories')}</h2>
        <p className="mb-3 text-xs text-zinc-400 dark:text-zinc-500">{t('menu.categories.desc')}</p>
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-full border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 py-1.5 ps-1.5 pe-3 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
            <label className="group relative h-7 w-7 shrink-0 cursor-pointer" title={t('menu.upload_all_tile')}>
              {settings?.all_category_image_url ? (
                <img src={settings.all_category_image_url} alt="" className="h-7 w-7 rounded-lg object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
              ) : (
                <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-50 dark:bg-zinc-800/60 text-sm ring-1 ring-zinc-200 dark:ring-zinc-700">✨</span>
              )}
          ...
          {categories.map((c) => (
            <div key={c.id} className={`flex items-center gap-2 rounded-full border py-1.5 ps-1.5 pe-1.5 text-xs font-semibold ${c.is_active ? 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300' : 'border-dashed border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-400 dark:text-zinc-500'}`}>
              <label className="group relative h-7 w-7 shrink-0 cursor-pointer" title={t('menu.upload_photo')}>
                {c.image_url ? (
                  <img src={c.image_url} alt="" className={`h-7 w-7 rounded-lg object-cover ring-1 ring-zinc-200 dark:ring-zinc-700 ${c.is_active ? '' : 'opacity-40 grayscale'}`} />
                ) : (
                  <span className={`flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-50 dark:bg-zinc-800/60 text-sm ring-1 ring-zinc-200 dark:ring-zinc-700 ${c.is_active ? '' : 'opacity-40'}`}>{c.icon}</span>
                )}
                <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-zinc-900/0 text-transparent transition group-hover:bg-zinc-900/40 group-hover:text-white">
              ...
              <button onClick={() => removeCategory(c)} className="text-zinc-300 dark:text-zinc-600 hover:text-red-500"><Trash2 size={12} /></button>
            </div>
          ))}
          <div className="flex items-center gap-1.5">
            <label className="group relative h-7 w-7 shrink-0 cursor-pointer" title={t('menu.optional_photo')}>
              {newCat.image_url ? (
                <img src={newCat.image_url} alt="" className="h-7 w-7 rounded-lg object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
              ) : (
                <span className="flex h-7 w-7 items-center justify-center rounded-lg border border-dashed border-zinc-300 text-zinc-300 dark:text-zinc-600">
                  <ImagePlus size={12} />
                </span>
              )}
              <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-zinc-900/0 text-transparent transition group-hover:bg-zinc-900/40 group-hover:text-white">
            ...
            <input value={newCat.icon} onChange={(e) => setNewCat({ ...newCat, icon: e.target.value })} className="w-12 rounded-lg border border-zinc-200 dark:border-zinc-700 px-2 py-1.5 text-center text-xs" maxLength={4} />
            <input value={newCat.name} onChange={(e) => setNewCat({ ...newCat, name: e.target.value })} placeholder={t('menu.new_category')} className="w-36 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-xs outline-none focus:border-brand-400" />
      ...
      {/* promo banners */}
      <div className="mb-6 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
        <h2 className="mb-1 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('menu.promo_banners')}</h2>
        <p className="mb-3 text-xs text-zinc-400 dark:text-zinc-500">{t('menu.promo_banners.desc')}</p>
        <div className="flex flex-wrap items-start gap-3">
          {promotions.map((p) => (
            <div key={p.id} className="flex flex-col items-center gap-1.5">
              <div className={`h-16 w-28 overflow-hidden rounded-xl ring-1 ring-zinc-200 dark:ring-zinc-700 ${p.is_active ? '' : 'opacity-40 grayscale'}`}>
                <img src={p.image_url} alt="" className="h-full w-full object-cover" />
              </div>
              <div className="flex items-center gap-1.5">
                <button onClick={() => togglePromotionActive(p)} title={p.is_active ? t('menu.hide_from_menu') : t('menu.show_on_menu')} className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${p.is_active ? 'bg-brand-50 text-brand-700' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500'}`}>
                  {p.is_active ? t('menu.visible') : t('menu.hidden')}
                </button>
                <button onClick={() => removePromotion(p)} className="text-zinc-300 dark:text-zinc-600 hover:text-red-500"><Trash2 size={11} /></button>
              </div>
            </div>
          ))}
          <label className="flex h-16 w-28 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500 transition hover:border-brand-300 hover:text-brand-500">
            {uploadingPromotion ? (
      ...
      {/* sauces */}
      <div className="mb-6 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
        <h2 className="mb-1 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('menu.sauces')}</h2>
        <p className="mb-3 text-xs text-zinc-400 dark:text-zinc-500">{t('menu.sauces.desc')}</p>
        <div className="flex flex-wrap items-start gap-3">
          {sauces.map((s) => (
            <div key={s.id} className="flex w-20 flex-col items-center gap-1.5 text-center">
              <label className="group relative h-16 w-16 cursor-pointer">
                {s.image_url ? (
                  <img src={s.image_url} alt="" className={`h-16 w-16 rounded-full object-cover ring-1 ring-zinc-200 dark:ring-zinc-700 ${s.is_active ? '' : 'opacity-40 grayscale'}`} />
                ) : (
                  <div className={`flex h-16 w-16 items-center justify-center rounded-full bg-zinc-50 dark:bg-zinc-800/60 text-zinc-300 dark:text-zinc-600 ring-1 ring-zinc-200 dark:ring-zinc-700 ${s.is_active ? '' : 'opacity-40'}`}>
                    <ImagePlus size={18} />
                  </div>
                )}
              ...
              <p className="truncate text-[11px] font-bold text-zinc-700 dark:text-zinc-300">{s.name}</p>
              {s.price > 0 && <p className="-mt-1 text-[10px] text-zinc-400 dark:text-zinc-500">+{money(s.price)}</p>}
              <button onClick={() => toggleSauceActive(s)} title={s.is_active ? t('menu.hide_from_menu') : t('menu.show_on_menu')} className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${s.is_active ? 'bg-brand-50 text-brand-700' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500'}`}>
                {s.is_active ? t('menu.visible') : t('menu.hidden')}
              </button>
              <button onClick={() => removeSauce(s)} className="text-zinc-300 dark:text-zinc-600 hover:text-red-500"><Trash2 size={11} /></button>
            </div>
          ))}
          {sauces.length === 0 && <p className="text-xs text-zinc-400 dark:text-zinc-500">{t('menu.no_sauces')}</p>}
        </div>
        ...
        <div className="mt-4 flex items-end gap-2 border-t border-zinc-50 pt-3">
          <label className="group relative h-12 w-12 shrink-0 cursor-pointer">
            {newSauce.image_url ? (
              <img src={newSauce.image_url} alt="" className="h-12 w-12 rounded-full object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
            ) : (
              <div className="flex h-12 w-12 items-center justify-center rounded-full border border-dashed border-zinc-300 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-300 dark:text-zinc-600">
                <ImagePlus size={16} />
              </div>
            )}
            <input
          ...
          <input value={newSauce.name} onChange={(e) => setNewSauce({ ...newSauce, name: e.target.value })} placeholder={t('menu.new_sauce')} className="w-32 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-xs outline-none focus:border-brand-400" />
          <input value={newSauce.price} onChange={(e) => setNewSauce({ ...newSauce, price: e.target.value })} type="number" step="0.10" min="0" placeholder={t('menu.extra_price')} className="w-20 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-xs outline-none focus:border-brand-400" />
      ...
      {/* supplements */}
      <div className="mb-6 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
        <h2 className="mb-1 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{t('menu.supplements')}</h2>
        <p className="mb-3 text-xs text-zinc-400 dark:text-zinc-500">{t('menu.supplements.desc')}</p>
        <div className="flex flex-wrap items-start gap-3">
          {supplements.map((s) => (
            <div key={s.id} className="flex w-20 flex-col items-center gap-1.5 text-center">
              <label className="group relative h-16 w-16 cursor-pointer">
              ...
              <p className="truncate text-[11px] font-bold text-zinc-700 dark:text-zinc-300">{s.name}</p>
              {s.price > 0 && <p className="-mt-1 text-[10px] text-zinc-400 dark:text-zinc-500">+{money(s.price)}</p>}
              <button onClick={() => toggleSupplementActive(s)} title={s.is_active ? t('menu.hide_from_menu') : t('menu.show_on_menu')} className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${s.is_active ? 'bg-brand-50 text-brand-700' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500'}`}>
                {s.is_active ? t('menu.visible') : t('menu.hidden')}
              </button>
              <button onClick={() => removeSupplement(s)} className="text-zinc-300 dark:text-zinc-600 hover:text-red-500"><Trash2 size={11} /></button>
            </div>
          ))}
          {supplements.length === 0 && <p className="text-xs text-zinc-400 dark:text-zinc-500">{t('menu.no_supplements')}</p>}
        </div>

        <div className="mt-4 flex items-end gap-2 border-t border-zinc-50 pt-3">
          <label className="group relative h-12 w-12 shrink-0 cursor-pointer">
            {newSupplement.image_url ? (
              <img src={newSupplement.image_url} alt="" className="h-12 w-12 rounded-full object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
            ) : (
              <div className="flex h-12 w-12 items-center justify-center rounded-full border border-dashed border-zinc-300 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-300 dark:text-zinc-600">
                <ImagePlus size={16} />
              </div>
          ...
          <input value={newSupplement.name} onChange={(e) => setNewSupplement({ ...newSupplement, name: e.target.value })} placeholder={t('menu.new_supplement')} className="w-32 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-xs outline-none focus:border-brand-400" />
          <input value={newSupplement.price} onChange={(e) => setNewSupplement({ ...newSupplement, price: e.target.value })} type="number" step="0.10" min="0" placeholder={t('menu.extra_price')} className="w-20 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-xs outline-none focus:border-brand-400" />
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {products.map((p) => (
          <div key={p.id} className="flex gap-3 rounded-2xl bg-white dark:bg-zinc-900 p-3 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-zinc-400 dark:text-zinc-500">{catName(p.category_id)} · {t('menu.stock')} {p.stock}</p>
              <div className="mt-2 flex items-center gap-1.5">
                <button
                  onClick={() => toggleAvailable(p)}
                  className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${p.is_available ? 'bg-brand-50 text-brand-700' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500'}`}
                >
                  {p.is_available ? t('menu.available') : t('menu.hidden')}
                </button>
                <button onClick={() => openEdit(p)} className="ms-auto rounded-lg p-1.5 text-zinc-400 dark:text-zinc-500 hover:bg-zinc-50 hover:text-brand-600"><Pencil size={14} /></button>
                <button onClick={() => removeProduct(p)} className="rounded-lg p-1.5 text-zinc-400 dark:text-zinc-500 hover:bg-red-50 hover:text-red-500"><Trash2 size={14} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? t('menu.edit_product') : t('menu.new_product')}>
        <div className="space-y-3">
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t('menu.product_name')} className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder={t('menu.description')} rows={2} className="w-full resize-none rounded-xl border border-zinc-200 dark:border-zinc-700 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.price')}</label>
              <input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} type="number" step="0.10" min="0" className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3 py-2.5 text-sm outline-none focus:border-brand-400" />
            </div>
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.stock_label')}</label>
              <input value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} type="number" min="0" className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3 py-2.5 text-sm outline-none focus:border-brand-400" />
            </div>
            <div>
              <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.category')}</label>
              <select value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })} className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-2 py-2.5 text-sm outline-none focus:border-brand-400">
          ...
          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.photo')}</label>
            <input
              value={form.image_url}
              onChange={(e) => setForm({ ...form, image_url: e.target.value })}
              placeholder={t('menu.photo.placeholder')}
              className="mb-2 w-full rounded-xl border border-zinc-200 dark:border-zinc-700 px-3.5 py-2.5 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100"
            />
            <div className="flex items-center gap-3">
              {form.image_url ? (
                <img src={form.image_url} alt="" className="h-14 w-14 rounded-xl object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
              ) : (
                <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-zinc-50 dark:bg-zinc-800/60 text-zinc-300 dark:text-zinc-600"><ImagePlus size={20} /></div>
              )}
              <label className="cursor-pointer rounded-xl border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50">
          ...
          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.gallery')}</label>
            <div className="flex flex-wrap items-center gap-2">
              {gallery.map((img) => (
                <div key={img.id} className="group relative h-16 w-16 shrink-0">
                  <img src={img.url} alt="" className="h-16 w-16 rounded-xl object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
                  <button
              ...
              {pendingGallery.map((url) => (
                <div key={url} className="group relative h-16 w-16 shrink-0">
                  <img src={url} alt="" className="h-16 w-16 rounded-xl object-cover ring-1 ring-zinc-200 dark:ring-zinc-700" />
                  <button
              ...
              <label className="flex h-16 w-16 shrink-0 cursor-pointer flex-col items-center justify-center gap-0.5 rounded-xl border border-dashed border-zinc-300 text-zinc-400 dark:text-zinc-500 hover:bg-zinc-50">
                <ImagePlus size={16} />
            ...
            <p className="mt-1.5 text-[10px] text-zinc-400 dark:text-zinc-500">{t('menu.gallery.desc')}</p>
          </div>

          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.sauces_offered')}</label>
            {sauces.length === 0 ? (
              <p className="text-xs text-zinc-400 dark:text-zinc-500">{t('menu.no_sauces_catalog')}</p>
            ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-full border-2 py-1.5 ps-1.5 pe-3 text-xs font-bold transition ${
                        active ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:border-zinc-200'
                      } ${s.is_active ? '' : 'opacity-50'}`}
                    >
                      {s.image_url ? (
                        <img src={s.image_url} alt="" className="h-6 w-6 rounded-full object-cover" />
                      ) : (
                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500"><Droplet size={12} /></span>
                      )}
            ...
            <p className="mt-1.5 text-[10px] text-zinc-400 dark:text-zinc-500">{t('menu.sauces_pick_hint')}</p>
          </div>

          <div>
            <label className="mb-1 block text-[10px] font-bold uppercase text-zinc-400 dark:text-zinc-500">{t('menu.supplements_offered')}</label>
            {supplements.length === 0 ? (
              <p className="text-xs text-zinc-400 dark:text-zinc-500">{t('menu.no_supplements_catalog')}</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {supplements.map((s) => {
                  ...
                  return (
                    ...
                    >
                      {s.image_url ? (
                        ...
                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500"><Layers size={12} /></span>
                      )}
                      {s.name}{s.price > 0 && <span className="opacity-60">+{money(s.price)}</span>}
                    </button>
                  );
            ...
            <p className="mt-1.5 text-[10px] text-zinc-400 dark:text-zinc-500">{t('menu.supplements_pick_hint')}</p>
          </div>
