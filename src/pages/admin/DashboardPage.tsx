...
export default function DashboardPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-5">
        <h1 className="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-100">{t('dashboard.title')}</h1>
      ...
      {!stats ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            {cards.map((c) => (
              <div key={c.labelKey} className="rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
                <div className={`mb-3 inline-flex h-9 w-9 items-center justify-center rounded-xl ${c.tint}`}>
                  <c.Icon size={17} />
                </div>
                <p className="font-display text-xl font-bold text-zinc-900 dark:text-zinc-100">{c.value}</p>
                <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500">{t(c.labelKey)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-3">
            {typeCards.map((tc) => (
              <div key={tc.labelKey} className="flex items-center gap-3 rounded-2xl bg-white dark:bg-zinc-900 p-4 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
                <div className={`flex h-9 w-9 items-center justify-center rounded-xl text-white ${tc.cls}`}>
                  <tc.Icon size={16} />
                </div>
                <div>
                  <p className="font-display text-lg font-bold leading-none text-zinc-900 dark:text-zinc-100">{tc.count}</p>
                  <p className="mt-0.5 text-[11px] font-medium text-zinc-400 dark:text-zinc-500">{t(tc.labelKey)} {t('dashboard.today_suffix')}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* live feed */}
      <div className="mt-6 rounded-2xl bg-white dark:bg-zinc-900 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
        <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-[15px] font-bold text-zinc-900 dark:text-zinc-100">{t('dashboard.live_feed')}</h2>
          </div>
        ...
        {loading ? (
          ...
          <p className="px-5 py-10 text-center text-sm text-zinc-400 dark:text-zinc-500">{t('dashboard.no_orders_yet')}</p>
        ) : (
          <ul className="divide-y divide-zinc-50">
            {orders.slice(0, 8).map((o) => (
              <li key={o.id} className="flex items-center gap-3 px-5 py-3">
                <span className="w-14 shrink-0 font-display text-sm font-bold text-zinc-900 dark:text-zinc-100">{orderNumber(o.id)}</span>
                <OrderTypeTag type={o.order_type} />
                <span className="hidden truncate text-xs text-zinc-400 dark:text-zinc-500 sm:inline">{orderContext(o)}</span>
                <span className="ms-auto text-sm font-bold text-zinc-900 dark:text-zinc-100">{money(o.total)}</span>
                <StatusBadge status={o.status} />
                <span className="hidden w-16 text-end text-[11px] text-zinc-400 dark:text-zinc-500 md:inline">{timeAgo(o.created_at)}</span>
              </li>
            ))}
          </ul>
        )}
